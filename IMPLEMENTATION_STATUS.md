...
- A repository-local Python 3.13 environment is used for checks; the package supports Python 3.11+.
...
| 15 - Docker, CI/CD, Handoff | Complete with external production validation pending | Auto-migration/non-root writable backend image; persistent app data volume; healthy Compose dependencies; worker/scheduler health and migration ordering; real API demo initializer; PowerShell and POSIX one-command startup; CI migration/container/login smoke; README/architecture/API/evaluation/experiments/deployment/security/contribution/roadmap/license; 12 module learning documents plus 4 defense/data-flow documents; honest demo seed with optional real index/evaluation pipeline | local `docker compose config`; rebuilt Compose health; PostgreSQL/Redis/Qdrant checks; worker ping; scheduler logs; Nginx/frontend smoke; current full regression | `183` backend tests; `13` Vitest tests; `9` fixture Playwright passed and `1` live gate skipped; local Compose all healthy | Coverage baseline remains historical; this checkpoint did not regenerate coverage report | Local development stack is healthy; production profile, persistence recovery, backup/restore, and release image provenance remain unverified; repository has no checkpoint commit because `.git` writes are restricted | Production credentials, target host, backup/restore, remote providers | Execute the remaining provider/source/live-browser gates in `PRODUCTION_ACCEPTANCE_CHECKLIST.md` |
...
- Git checkpoint commits require elevated `.git` write access in this sandbox.
...
- The repository is intentionally left without a checkpoint commit/tag; all files are currently untracked in this sandbox and must be committed through the normal release workflow.
