...
export async function installApiFixtures(page: Page): Promise<void> {
  ...
  await page.route(/^https?:\/\/[^/]+\/api\//, async (route) => {
    ...
    if (path.match(/^\/crawl-tasks\/\d+\/acceptance-summary$/) && method === 'GET') return fulfill(route, { crawl_task_id: 1, database_document_count: 2, chunk_count: 4, qdrant_point_count: 4 })
